...
async def get_spotify_access_token(session: aiohttp.ClientSession) -> str:
    ...
    Note:
        The function uses the 'client_credentials' grant type for authentication, which is suitable
...
def get_spotify_song_urls(html: str) -> list[str]:
    ...
    content, the function extracts and returns the first 10 song URLs.
...
def get_relevant_spotify_song_response_data(response: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    Note:
        The function assumes that the response dictionary has a specific structure as provided by
        the Spotify Web API. Changes in the Spotify Web API could affect the function's output and
...
async def get_spotify_song_data_from_spotify_song_urls(
    ...
    Note:
        The function depends on the 'fetch_spotify' function to make individual API requests and
        'get_relevant_spotify_song_response_data' to extract relevant data from each response.
        Changes in the Spotify Web API could affect the function's output and may require updates to
        the request logic or data extraction process.
    """
...
async def get_spotify_song_data_from_isrcs(
    ...
    the function makes concurrent asynchronous requests, one for each ISRC. If a song for an ISRC
    ...
    Note:
        ...
        the request logic or data extraction process.
    """
    # There is no capability to search for multiple ISRCs, unlike for songs (tracks)
    data = await asyncio.gather(
        *[
            fetch_spotify(
                session,
                f"{SPOTIFY_BASE_API_URL}v1/search?q=isrc:{isrc}&type=track",
                spotify_access_token,
            )
            for isrc in isrcs
        ]
...
async def load_ranking_data(connection: asyncpg.Connection, isrcs: list[str]) -> None:
    ...
    Note:
        The function assumes the ISRCs are provided in a specific order - the first half corresponds
        to Spotify rankings and the second half to Apple Music. This ordering affects how the 'rank'
        and 'ranking_source' fields are determined.
    """
    successful_records = await connection.fetch(
        """
        INSERT INTO
...
async def main() -> None:
    ...
    async with (
        ...
        print("GREEPPPP")
        print(corrected_apple_music_song_isrcs)
        print(spotify_song_data.keys())
        print(apple_music_song_data.keys())
